		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for i := 0; i < len(tokens); i++ {
				...
				rows = append(rows, []format2.Value{
					format2.String(ingestToken.Name),
					format2.String(ingestToken.Token),
					format2.ValueOrEmpty(ingestToken.AssignedParser),
				})
			}

			format2.PrintOverviewTable(cmd, []string{"Name", "Token", "Assigned Parser"}, rows)
		},
	}
